    ...
    <div className="grid lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 bg-white rounded-2xl border border-preto/10 overflow-hidden">
        <table className="w-full text-left text-sm">
          ...
          <tbody>{loading && <tr><td className="px-5 py-4" colSpan={2}>Carregando...</td></tr>}
            {list.map((u)=>(<tr key={u.id} className="border-t border-preto/5"><td className="px-5 py-3 font-medium">{u.email}</td><td className="text-preto/50">{new Date(u.created_at).toLocaleDateString('pt-BR')}</td></tr>))}
          </tbody>
        </table>
      </div>
    </div>
