...
export default function Processos() {
  ...
  const carregar = async () => { const r = await fetch('/api/processes'); setList(await r.json()); setLoading(false); };
  useEffect(() => { supabase.auth.getUser().then(({ data }) => { if (!data.user) window.location.href='/login'; else carregar(); }); }, []);
  const criar = async () => {
    await fetch('/api/processes', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ type: tipo }) });
    carregar();
  };

  return (
    <Shell title="Processos">
      <div className="flex gap-2 mb-6">
        <select value={tipo} onChange={(e)=>setTipo(e.target.value)} className="border border-preto/15 rounded-xl px-3 py-2 bg-white">
      ...
      <div className="bg-white rounded-2xl border border-preto/10 overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-offwhite text-preto/60">
            <tr><th className="px-5 py-3">Número</th><th>Tipo</th><th>Status</th><th>Criado</th></tr>
          </thead>
          <tbody>
            ...
            {list.map((p)=>(
              <tr key={p.id} className="border-t border-preto/5">
                <td className="px-5 py-3 font-semibold">{p.number}</td>
                <td>{LABEL[p.type] ?? p.type}</td>
                <td><span className="bg-offwhite px-2 py-1 rounded-lg text-xs">{p.status}</span></td>
                <td className="text-preto/50">{new Date(p.created_at).toLocaleString('pt-BR')}</td>
              </tr>
            ))}
          </tbody>
        </table>
